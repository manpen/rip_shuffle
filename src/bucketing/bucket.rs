...
impl<'a, T> Bucket<'a, T> {
    ...
    pub fn shrink_to_right(&mut self, rhs: &mut Self, num: usize) {
        ...
        // it holds this_bucket.num_unprocessed() >= too_long_by >= to_move
...
mod test {
                ...
                for left_shrinkage in 0..left_stash_len {
                    for right_stash_len in 0..right_len {
                        let (mut left, mut right) = {
                            ...
                            (Bucket::new(left), Bucket::new(right))
                        };
    ...
    fn len() {
        for i in 0..4 {
            ...
            let bucket = Bucket::new(&mut data);

            assert_eq!(bucket.len(), i);
            assert_eq!(bucket.is_empty(), i == 0);
        }
    }
    ...
    fn is_fully_processed() {
        for i in 0usize..4 {
            ...
            for j in 0..i {
                assert!(!bucket.is_fully_processed());
                assert_eq!(bucket.num_processed(), j);
                assert_eq!(bucket.num_unprocessed(), i - j);
                assert_eq!(*bucket.first_unprocessed().unwrap(), j);
                assert_eq!(bucket.process_element().is_some(), j + 1 < i);
            }
    ...
    fn data() {
        ...
        for i in 0usize..4 {
            ...
            for j in 0..i {
                *bucket.first_unprocessed().unwrap() = j + 1;
                bucket.process_element();

                // data
                assert_eq!(Vec::from(bucket.data()), ref_vec(i, j));
                assert_eq!(Vec::from(bucket.data_mut()), ref_vec(i, j));

                // data_processed
                assert_eq!(bucket.data_processed().len(), j + 1);
                assert_eq!(bucket.data_processed_mut().len(), j + 1);
                assert!(bucket
                    .data_processed()
                    .iter()
                    .enumerate()
                ...
                // data_unprocessed
                assert_eq!(bucket.data_unprocessed().len(), i - j - 1);
                assert_eq!(bucket.data_unprocessed_mut().len(), i - j - 1);
                assert!(bucket.data_unprocessed().iter().all(|x| *x == 0));
                assert!(bucket.data_unprocessed_mut().iter().all(|x| *x == 0));
            }
        ...
        for total_len in [1usize, 2, 3, 5, 8, 12] {
            for left_len in 0..total_len {
                ...
                for (left_stash, right_stash) in (0..=left_len).cartesian_product(0..=right_len) {
                    ...
                    let mut left_bucket = Bucket::new_with_num_unprocessed(left_data, left_stash);
                    let mut right_bucket =
                        Bucket::new_with_num_unprocessed(right_data, right_stash);

                    left_bucket.data_processed_mut().fill(1);
                    right_bucket.data_processed_mut().fill(2);

                    let merged = left_bucket.merge_with_right_neighbor(right_bucket);
